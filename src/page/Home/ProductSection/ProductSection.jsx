...
export default function ProductSection() {
  ...
  // Sau này bạn thay console.log bằng navigate('/product/' + p.id)
  const handleOpenDetail = (product) => {
    console.log("Go to product detail:", product.id, product.name);
  };

  const handleAddToCart = (product, e) => {
    e.stopPropagation(); // phòng khi sau này bạn cho cả card clickable
    ...
    <section className="border-b border-white/5 bg-[#020D07]">
      <div className="container px-4 xl:px-0 py-[72px] flex flex-col gap-9">
        ...
        <div className="border-b border-white/10 overflow-x-auto scrollbar-none">
          <div className="flex min-w-max gap-x-6 md:gap-x-6 pl-4 md:pl-[72px]">
            {TABS.map((tab) => {
